...
(function () {
  ...
  function useIn(target) {
    ...
    sb.from('research_files').upsert({ project_id: projId(), path: path, content: content, mime: 'text/markdown', size: content.length, source: 'ai', created_by: uid, updated_by: uid, updated_at: new Date().toISOString() }, { onConflict: 'project_id,path' }).then(function (r) {
      if (r && r.error) { toast('Could not save: ' + r.error.message, false); return; }
      toast('✓ Saved to your ' + (target === 'protocol' ? 'Protocol' : 'Writing') + ' documents (' + path + ')');
    });
  }
  ...
  function wire() {
    ...
    var up = document.getElementById('useProt'); if (up) up.onclick = function () { useIn('protocol'); };
    var uw = document.getElementById('useWrite'); if (uw) uw.onclick = function () { useIn('writing'); };
  }
