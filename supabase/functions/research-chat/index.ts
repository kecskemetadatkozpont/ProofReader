...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Research-radar fence — the client highlights these spans and (later) verifies them against OpenAlex. 0 extra API calls:
    // it rides the same completion. Keep it stable (it's inside the cached prefix).
    const SPANS_NOTE = ` If (and ONLY if) your reply genuinely proposes a research IDEA or names a research GAP, you MAY ADD — as the VERY LAST block, after any other block — a fenced JSON array marking those spans, in EXACTLY this form:\n\`\`\`publify-spans\n[{"type":"gap","quote":"<a VERBATIM substring copied EXACTLY from your reply above, ≤120 chars, that states the gap/idea>","keywords":["<2–4 English search keywords for OpenAlex>"],"strength":"strong"}]\n\`\`\`\n"type" is "idea" (a novel research direction/question you proposed) or "gap" (an under-studied area you named). "quote" MUST be an exact character-for-character substring of your reply's prose (so the client can find and highlight it) — never paraphrase it, never quote the questions block. "keywords" are English topical terms to search the literature. "strength" is "strong" or "weak". Include at most 5, only the genuine ones. Omit the whole block when your reply proposes no idea/gap. This block is silent (the user never sees the JSON).`;
    const SYSTEM = persona + mcpNote + ATTACH_NOTE + FILE_NOTE + QUESTIONS_NOTE + SPANS_NOTE;
