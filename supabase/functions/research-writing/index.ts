...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const gate = await assertEntitled(sb, 'research_ai_writing'); if (gate) return gate;
    const model = await clampModel(sb, 'claude-opus-4-8');
    ...
    if (action === 'outline') {
      ...
      const raw = await callClaude(sys, u, 3000, model); await sb.rpc('ai_usage_bump');
      ...
      return json({ ok: true, outline: ol, context, model });
    }
    ...
    if (action === 'section') {
      ...
      const latex = await callClaude(sys, u, 3500, model); await sb.rpc('ai_usage_bump');
