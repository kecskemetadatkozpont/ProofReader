...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const gate = await assertEntitled(sb, 'page_media'); if (gate) return gate;
    ...
    if (mode === 'summarize') {
      ...
      const text = await callClaude(sb, sys, list, 4000); await sb.rpc('ai_usage_bump');
      return json({ text });
    }
    ...
    if (mode === 'fulltext') {
      ...
      const text = await callClaude(sb, sys, [pdf, { type: 'text', text: 'Return the clean reading text now.' }], 8000); await sb.rpc('ai_usage_bump');
      return json({ text });
    }
    ...
    const out = await callClaude(sb, sys, 'Segments (JSON array of strings):\n' + JSON.stringify(segments), 8000); await sb.rpc('ai_usage_bump');
    let arr: any = [];
