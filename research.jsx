  ...
  function PipelineCanvas(props) {
    ...
    function loadMembers() {
      sb.from('research_project_members').select('user_id,role,accepted,invited_by,created_at').eq('project_id', props.projectId).then(function (r) {
        ...
        sb.from('profiles_public').select('id,name,avatar_url').in('id', fetchIds).then(function (pr) {   // profiles_public exposes avatar_url (NOT avatar) — selecting a missing column 42703-errors the whole query → names fell back to "Kolléga"
          if (!alive.current) return; var byId = {}; ((pr && pr.data) || []).forEach(function (p) { byId[p.id] = { name: p.name, avatar: p.avatar_url }; });
          setPeopleById(byId);
