...
(function () {
  ...
  // 🕰 Idővonal (timeline) mode lane labels + the by-Type lane grouping + a short date formatter
  ...
  function PipelineCanvas(props) {
    ...
    var tpS = useState(false), tlPlay = tpS[0], setTlPlay = tpS[1];   // replay playing
    var tfS = useState(true), tlFollow = tfS[0], setTlFollow = tfS[1];   // v2 follow-cam: pan the camera to keep the playhead centered during replay
    var tlRaf = useRef(0), tlLast = useRef(0), tlCurRef = useRef(1), tlPlayRef = useRef(false), tlFollowRef = useRef(true);   // rAF handle + last-frame ts + live cursor mirror + playing flag + follow-cam flag (for the pan-pause + play loop)
    ...
    var hfS = useState(null), hovFrame = hfS[0], setHovFrame = hfS[1];   // P1.5: frame id whose header is hovered → preview which cards it will carry
    ...
    var pplS = useState({}), peopleById = pplS[0], setPeopleById = pplS[1];   // uid → {name, avatar} for creator badges (members + the owner, who isn't in the members table)
    ...
    useEffect(function () {
      ...
      function onKey(e) {
        if (e.key === 'Escape') { setRadial(null); setArcOpen(null); if (viewMenu || arrMenu || zoomMenu) closeDMenus(); else if (tour) tourStop(); else if (focus) exitFocus(); else if (tlOn || phaseArr) tlExit(); else { setLinkFrom(null); setSelEdge(null); } }   // Esc closes: dock menus → radial/arc → tour → focus → timeline/phase → link/edge selection
    ...
    }, [focus, tour, tlOn, phaseArr, viewMenu, arrMenu, zoomMenu]);   // re-bind so Esc reads the live mode/menu flags (not a stale closure)
    ...
    useEffect(function () {
      var pid = props.projectId;
      Promise.all([
        sb.from('research_ideas').select('id,question,hypothesis,rationale,novelty,status,source,gap_type,evidence,addressed_by_idea_id,created_by,created_at').eq('project_id', pid).neq('status', 'rejected').order('created_at', { ascending: true }).limit(24).then(function (r) { return (r && r.error) ? sb.from('research_ideas').select('id,question,hypothesis,rationale,novelty,status,source,created_by,created_at').eq('project_id', pid).neq('status', 'rejected').order('created_at', { ascending: true }).limit(24) : r; }),   // self-gate: on a missing-gap-column error, resolve to the base-column select so ideas never vanish pre-migration
        sb.from('research_studies').select('id,idea_id,title,question,status,created_by,created_at').eq('project_id', pid).order('created_at', { ascending: true }),
    ...
    function graph() {
      ...
      // 🕰 timeline timestamps: n.ts (epoch ms) from the source row's created_at (chat = updated_at); aggregates/derived get an estimate.
      N.forEach(function (n) { var r = n.ref, s = r ? (n.t === 'chat' ? (r.updated_at || r.created_at) : (r.created_at || r.updated_at)) : null; var t = s ? Date.parse(s) : NaN; n.ts = isNaN(t) ? null : t; });
      (function () {   // fill null-ts nodes (lit/sr aggregates, undated) from the DAG: min(children)-eps else max(parents)+eps; iterate to a fixpoint
        var par = {}, kid = {}, byId = {}; N.forEach(function (n) { par[n.id] = []; kid[n.id] = []; byId[n.id] = n; });
        E.forEach(function (e) { if (par[e[1]] && kid[e[0]]) { par[e[1]].push(e[0]); kid[e[0]].push(e[1]); } });
        var HR = 3600000;
        for (var it = 0; it < 6; it++) {
          var ch = false;
          N.forEach(function (n) {
            if (n.ts != null) return;
            var ks = kid[n.id].map(function (id) { return byId[id] && byId[id].ts; }).filter(function (t) { return t != null; });
            var ps = par[n.id].map(function (id) { return byId[id] && byId[id].ts; }).filter(function (t) { return t != null; });
            var nt = ks.length ? Math.min.apply(null, ks) - HR : (ps.length ? Math.max.apply(null, ps) + HR : null);
            if (nt != null) { n.ts = nt; n.tsEst = true; ch = true; }
          });
          if (!ch) break;
        }
      })();
    ...
    function onDown(e) {
      tlPause();   // 🕰 panning pauses the timeline replay (does NOT exit the mode)
      if ((viewMenu || arrMenu || zoomMenu) && !(e.target.closest && e.target.closest('.rmap-zoom'))) closeDMenus();   // a CANVAS click closes any open dock menu — but a click on the dock itself must NOT (its own trigger toggles via openDMenu; else onDown closes then onClick reopens)
    ...
    function openDMenu(which, e) {   // toggle one dock menu, close the others, anchor the popover under the clicked trigger
      var st = stageRef.current, tgt = e && e.currentTarget;
      if (st && tgt && tgt.getBoundingClientRect) { var r = tgt.getBoundingClientRect(), sr = st.getBoundingClientRect(); dmenuAnchor.current = { cx: r.left + r.width / 2 - sr.left, bottom: sr.bottom - r.top + 8 }; }
      setViewMenu(function (v) { return which === 'view' ? !v : false; });
      setArrMenu(function (v) { return which === 'arr' ? !v : false; });
      setZoomMenu(function (v) { return which === 'zoom' ? !v : false; });
    }
    // fit the whole graph into the viewport (Luma "illeszd a nézetbe"). Uses card size 204x74.
    function fitView() {
      tlPause();   // a manual fit (Nézetbe) must not be clobbered by the next follow-cam frame
    ...
    function groupIntoFrame() {
      ...
      // P1.5 — dedup: if a frame already hugs this exact bbox (regrouping the same cards), update it instead of stacking a new one
      var dup = frames.filter(function (fr) { return Math.abs(fr.x - fx) < 10 && Math.abs(fr.y - fy) < 10 && Math.abs(fr.w - fw) < 10 && Math.abs(fr.h - fh) < 10; })[0];
      if (dup) { framePatch(dup.id, { x: fx, y: fy, w: fw, h: fh }); window.PRUI.toast('Már van keret e köré — frissítve', { kind: 'info' }); setMsel({}); return; }
    ...
    function nameOf(uid) {
      ...
      if (peopleById[uid] && peopleById[uid].name) return peopleById[uid].name;
    ...
    function avatarOf(uid) {   // profile picture for a creator/collaborator badge (from members/owner or presence), else null → colored initial
      if (!uid) return null; if (uid === props.viewerId) return (props.viewer && props.viewer.avatar) || null;
      if (peopleById[uid] && peopleById[uid].avatar) return peopleById[uid].avatar;
    ...
    function monogram(nm) { var p = String(nm || '').trim().split(/\s+/).filter(Boolean); if (!p.length) return '?'; return (p.length === 1 ? p[0].slice(0, 2) : (p[0].charAt(0) + p[p.length - 1].charAt(0))).toUpperCase(); }   // 2-letter initials (e.g. "Szabo Lorant" → "SL")
    function colorOf(uid) { return (peopleById[uid] && peopleById[uid].color) || userColor(uid); }   // ONE canonical per-user color (profiles_public.color), shared by the map badge, presence dots + share avatars
    ...
    function loadMembers() {
      sb.from('research_project_members').select('user_id,role,accepted,invited_by,created_at').eq('project_id', props.projectId).then(function (r) {
        ...
        var oid = props.project && props.project.owner_id, fetchIds = ids.slice(); if (oid && fetchIds.indexOf(oid) < 0) fetchIds.push(oid);   // include the owner so their cards attribute correctly
        sb.from('profiles_public').select('id,name,avatar_url,color').in('id', fetchIds).then(function (pr) {   // profiles_public exposes avatar_url (NOT avatar) — selecting a missing column 42703-errors the whole query → names fell back to "Kolléga"
          if (!alive.current) return; var byId = {}; ((pr && pr.data) || []).forEach(function (p) { byId[p.id] = { name: p.name, avatar: p.avatar_url, color: p.color }; });
          setPeopleById(byId);
    ...
    function startFrameDrag(e, f, mode) {   // mode: 'move' (titlebar) | 'resize' (corner handle)
      ...
      var carry = (mode === 'move') ? g.N.filter(function (n) { return !n.mapHidden && !(e.shiftKey && n.mapPinned) && inFrame(n, f); }) : [];   // P1.5: Shift+drag leaves pinned cards behind
    ...
    function timelineLayout(N, E, opts) {
      ...
      if (dated.length) {
        tMin = dated[0].ts; tMax = dated[dated.length - 1].ts; if (tMax <= tMin) tMax = tMin + DAYMS;
        if (xMode === 'linear') {
          var SPAN = Math.max(900, dated.length * 150);
          dated.forEach(function (n) { xs[n.id] = GUT + PAD + (n.ts - tMin) / (tMax - tMin) * SPAN; });
          var days = Math.max(1, Math.round((tMax - tMin) / DAYMS)), sd = Math.max(1, Math.round(days / 8));
          for (var kd = 0; kd <= days; kd += sd) ticks.push({ x: GUT + PAD + kd * DAYMS / (tMax - tMin) * SPAN, lab: '+' + kd + 'n' });
        } else if (xMode === 'ordinal') {
          var SP2 = Math.max(900, dated.length * 150);
          dated.forEach(function (n, i) { xs[n.id] = GUT + PAD + (dated.length < 2 ? 0 : i / (dated.length - 1)) * SP2; });
          var so = Math.max(1, Math.round(dated.length / 9));
          for (var io = 0; io < dated.length; io += so) ticks.push({ x: xs[dated[io].id], lab: '#' + (io + 1) });
        } else if (xMode === 'depth') {   // v2 Származás: x = DAG-generation from the roots (NOT the clock) — pure "what came from what"; longest-path via iterative relaxation (order-independent + cycle-clamped, so a manual cycle can't hang or give order-dependent columns)
          var gmemo = {}, cap = N.length; N.forEach(function (n) { gmemo[n.id] = 0; });
          for (var pass = 0; pass < cap; pass++) { var chg = false; E.forEach(function (e) { if (gmemo[e[0]] != null && gmemo[e[1]] != null) { var cand = gmemo[e[0]] + 1; if (cand > gmemo[e[1]] && cand <= cap) { gmemo[e[1]] = cand; chg = true; } } }); if (!chg) break; }
          var maxg = 0; N.forEach(function (n) { if (gmemo[n.id] > maxg) maxg = gmemo[n.id]; });
          var COLW = 250; N.forEach(function (n) { xs[n.id] = GUT + PAD + gmemo[n.id] * COLW; });
          for (var gd = 0; gd <= maxg; gd++) ticks.push({ x: GUT + PAD + gd * COLW, lab: 'Gen ' + gd });
        } else {   // squished: bucket by day, width ∝ count, fixed gap, fan within a bucket
          var byDay = {}; dated.forEach(function (n) { var dd = Math.floor((n.ts - tMin) / DAYMS); (byDay[dd] = byDay[dd] || []).push(n); });
          var keys = Object.keys(byDay).map(Number).sort(function (a, b) { return a - b; }), cx = GUT + PAD;
          keys.forEach(function (dd) {
            var grp = byDay[dd].sort(function (a, b) { return a.ts - b.ts; }), bw = Math.max(160, grp.length * 110);
            grp.forEach(function (n, i) { xs[n.id] = cx + (grp.length < 2 ? bw / 2 : (i + 0.5) / grp.length * bw); });
            ticks.push({ x: cx, lab: tlFmtDay(tMin + dd * DAYMS) }); cx += bw + 30;
          });
        }
      }
      if (xMode !== 'depth') undated.forEach(function (n) { xs[n.id] = GUT / 2; });   // undated → far-left gutter column (in depth mode they keep their DAG generation)
    ...
    function tlPlayLoop(ts) {
      if (!tlPlayRef.current) return;
      if (!tlLast.current) tlLast.current = ts;
      var dt = ts - tlLast.current; tlLast.current = ts;
      var nc = tlCurRef.current + dt / 9000;   // ~9s full sweep at 1×
      if (nc >= 1) { tlCurRef.current = 1; if (alive.current) setTlCur(1); tlStopPlay(); if (tlFollowRef.current && alive.current) tlFollowCam(1); return; }
      tlCurRef.current = nc; if (alive.current) setTlCur(nc);
      if (tlFollowRef.current && alive.current) tlFollowCam(nc);   // v2 follow-cam: keep the playhead centered
      tlRaf.current = requestAnimationFrame(tlPlayLoop);
    }
    function tlFollowCam(nc) {   // ease the camera so the playhead world-x sits ~42% from the left of the stage
      var m = g && g.tl, st = stageRef.current; if (!m || !st) return;
      if (m.xMode === 'depth') return;   // depth = generation-x, not time → no time-playhead to follow (would lurch over non-monotonic columns)
      var px = tlXForT(m.tMin + nc * (m.tMax - m.tMin)), sw = st.clientWidth || 900, v = viewRef.current;
      var targetTx = sw * 0.42 - px * v.k, ntx = v.tx + (targetTx - v.tx) * 0.12;
      if (Math.abs(ntx - v.tx) > 0.4) setView(function (vv) { return { tx: ntx, ty: vv.ty, k: vv.k }; });
    }
    function tlStartPlay() { if (!g || !g.tl) return; if (tlCurRef.current >= 1) { tlCurRef.current = 0; setTlCur(0); } tlPlayRef.current = true; setTlPlay(true); tlLast.current = 0; tlRaf.current = requestAnimationFrame(tlPlayLoop); }
    ...
    function arrSet(mode) {
      closeDMenus();   // a mode switch closes any open dock menu (the arrange caret unmounts in ephemeral modes)
    ...
    function tlXForT(t) {   // playhead world-x for a cursor time — interpolate over real-dated nodes' (ts → center-x)
      var m = g && g.tl; if (!m) return 0;
      var ds = g.N.filter(function (n) { return n.ts != null && !n.tsEst; }).sort(function (a, b) { return a.ts - b.ts; });
      if (!ds.length) return m.gut + 90;
      function cx(n) { return n.x + (n._w || 210) / 2; }
      if (t <= ds[0].ts) return cx(ds[0]);
      if (t >= ds[ds.length - 1].ts) return cx(ds[ds.length - 1]);
      for (var i = 0; i < ds.length - 1; i++) { if (t >= ds[i].ts && t < ds[i + 1].ts) { var f = (t - ds[i].ts) / ((ds[i + 1].ts - ds[i].ts) || 1); return cx(ds[i]) + (cx(ds[i + 1]) - cx(ds[i])) * f; } }
      return cx(ds[ds.length - 1]);
    }
    ...
    // decluttered dock (V1): the popover for whichever of the 3 dock menus is open (Nézet / Elrendezés-akciók / Nagyítás). Anchored under its trigger; items reuse the existing handlers.
    function renderDMenu() {
      var which = viewMenu ? 'view' : arrMenu ? 'arr' : zoomMenu ? 'zoom' : null; if (!which) return null;
      var a = dmenuAnchor.current;
      function item(ic, label, sub, onClick, opts) { opts = opts || {}; return h('button', { key: label, className: 'rmap-dmi' + (opts.danger ? ' danger' : '') + (opts.on ? ' on' : ''), disabled: !!opts.disabled, onClick: function () { closeDMenus(); onClick(); } }, h('span', { className: 'rmap-dmi-ic' }, ic), h('span', { className: 'rmap-dmi-t' }, label), sub ? h('span', { className: 'rmap-dmi-s' }, sub) : null); }
      function head(t) { return h('div', { key: 'h' + t, className: 'rmap-dmi-h' }, t); }
      function sep(k) { return h('div', { key: 'sep' + k, className: 'rmap-dmi-sep' }); }
      var items, mw;
      if (which === 'view') {
        mw = 232;
        var nHid = g.N.filter(function (n) { return n.mapHidden; }).length, hasExtra = (commentsCap && comments.length) || (data && data.hiddenFigs && data.hiddenFigs.length) || (mapFlags && nHid);
        items = [
          head('Nézet & panelek'),
          ...
          pathsCap ? item('🎬', 'Bemutatók', '', function () { setPresMgrOpen(function (v) { return !v; }); }) : null,
          hasExtra ? sep(2) : null,
          (commentsCap && comments.length) ? item('📋', 'Összes komment', cmUnresolved ? cmUnresolved + ' nyitott' : '', function () { setCmPanelOpen(function (v) { return !v; }); }) : null,
          (data && data.hiddenFigs && data.hiddenFigs.length) ? item('🖼', 'Rejtett ábrák', '' + data.hiddenFigs.length, function () { setRestoreOpen(true); }) : null,
          (mapFlags && nHid) ? item('🫥', 'Rejtett kártyák', '' + nHid, function () { setNodeRestoreOpen(true); }) : null
        ].filter(Boolean);
      } else if (which === 'arr') {
        mw = 238;
        items = [
          head('Elrendezés-akciók (mentett)'),
          (props.canEdit && cardSizeCap && g.N.length > 1) ? item('✨', 'Okos elrendezés', 'tartalom', smartLayout, { disabled: smartBusy }) : null,
          (props.canEdit && g.N.length > 1) ? item('🧹', 'Rendezd el', 'de-overlap', tidyLayout) : null,
          (props.canEdit && framesCap) ? sep(1) : null,
          (props.canEdit && framesCap) ? item('⌗', 'Fázisokba (végleges)', 'keretekkel', autoLayoutStages, { danger: true }) : null,
          (props.canEdit && Object.keys(layout).length) ? item('↺', 'Vissza auto-elrendezésre', '', resetLayout) : null,
          (props.canEdit && framesCap) ? sep(2) : null,
          (props.canEdit && framesCap) ? item('▦', 'Új keret', '', function () { frameCreate(); }) : null
        ].filter(Boolean);
      } else {
        mw = 196; var pk = Math.round(view.k * 100);
        items = [
          head('Nagyítás'),
          item('⤢', 'Illeszd a nézetbe', '', fitView),
          sep(1),
          item('', '50%', '', function () { setZoomTo(0.5); }, { on: pk === 50 }),
          item('', '100%', '', function () { setZoomTo(1); }, { on: pk === 100 }),
          item('', '150%', '', function () { setZoomTo(1.5); }, { on: pk === 150 }),
          item('', '200%', '', function () { setZoomTo(2); }, { on: pk === 200 })
        ];
      }
      var st = stageRef.current, sw = st ? st.clientWidth : 900, left = Math.max(mw / 2 + 8, Math.min(sw - mw / 2 - 8, a.cx));
      return h('div', { className: 'rmap-dmenu', style: { left: left + 'px', bottom: a.bottom + 'px', width: mw + 'px' }, onMouseDown: function (e) { e.stopPropagation(); }, onWheel: function (e) { e.stopPropagation(); } }, items);
    }
    function tlStepCard(dir) {   // ⏮/⏭ — jump the cursor to the prev/next card in creation order
      ...
      if (!arr.length) return; var t = tlCursorT(), idx;
      if (dir > 0) { idx = arr.findIndex(function (v) { return v > t + 1; }); if (idx < 0) idx = arr.length - 1; }
      else { for (idx = arr.length - 1; idx >= 0 && arr[idx] >= t - 1; idx--) { } if (idx < 0) idx = 0; }
      tlSetCur((m.tMax - m.tMin) ? (arr[idx] - m.tMin) / (m.tMax - m.tMin) : 1);
    }
    // v2 — snapshot the chronological walk into a saved Prezi story (research_map_paths). Node beats re-resolve to each card's
    // LIVE position at play time (resolveBeat → nodeTarget), so the saved story walks the cards in creation order regardless of layout.
    function tlSavePresentation() {
      if (!props.canEdit || !pathsCap || !g || !g.tl) return;
      var ordered = g.N.filter(function (n) { return n.ts != null && nodeVisible(n); }).sort(function (a, b) { return a.ts - b.ts; });
      if (!ordered.length) { window.PRUI.toast('Nincs időbélyeges kártya a bemutatóhoz.', { kind: 'info' }); return; }
      var steps = ordered.map(function (n) { var tab = RMAP_TYPE[n.t] && RMAP_TYPE[n.t].tab, v = nodeTarget(n, 1.4); return { kind: 'node', ref_id: n.id, tx: v.tx, ty: v.ty, k: v.k, caption: (n.title || (RMAP_TYPE[n.t] && RMAP_TYPE[n.t].lab) || ''), notes: (n.ts ? new Date(n.ts).toLocaleDateString('hu-HU') : '') + (n.tsEst ? ' (becsült)' : ''), enter_panel: false, panel_tab: tab, dwell_ms: 2600 }; });
      sb.from('research_map_paths').insert({ project_id: props.projectId, name: 'Idővonal — ' + new Date().toLocaleDateString('hu-HU'), ord: paths.length, steps: steps }).select('id,name,ord,steps').single().then(function (r) {
        if (!alive.current) return;
        if (r && r.error) { window.PRUI.toast('Mentés sikertelen: ' + r.error.message, { kind: 'error' }); return; }
        if (r && r.data) { setPaths(function (P) { return P.some(function (x) { return x.id === r.data.id; }) ? P : P.concat([r.data]); }); window.PRUI.toast(ordered.length + ' jelenetes bemutató mentve · nyisd meg a 🎬 Bemutatók közül', { kind: 'success' }); }
      });
    }
    ...
    function pvSelUp(n) {   // text selected inside a file card's md/text preview → offer to add it to the chat as context
      setTimeout(function () {
        var s = window.getSelection ? window.getSelection() : null, t = s ? String(s).trim() : '';
        if (t && t.length > 3) { try { var rr = s.getRangeAt(0).getBoundingClientRect(); setPvSel({ text: t.slice(0, 4000), x: rr.left + rr.width / 2, y: rr.top, file: (n && (n.title || (n.ref && n.ref.path))) || 'fájl', fileId: n && n.ref && n.ref.id }); } catch (e) { setPvSel(null); } }
        else setPvSel(null);
      }, 1);
    }
    function pvAddSnippet() {   // add the selected preview snippet as a pending chat-context chip + open the dock chat
      if (!pvSel) return;
      setDSnips(function (p) { return p.concat([{ text: pvSel.text, file: pvSel.file, fileId: pvSel.fileId }]); });
      setDkOpen(true); try { localStorage.setItem('pr-rmap-dock', '1'); } catch (e) { }
      setDkTab('chat'); setPvSel(null);
      try { window.getSelection().removeAllRanges(); } catch (e) { }
    }
    ...
    function dkSend(overrideTxt, skipEcho) {   // free-text turn → research-chat (streaming); the reply may save files (→ file nodes). skipEcho: caller already echoed the user turn (frame-generation chat fallback)
      ...
      if (snips.length) full = '[KIJELÖLT SZÖVEGRÉSZLET(EK) — a felhasználó ezeken a részleteken akar dolgozni, a megadott fájlon belül:\n' + snips.map(function (s) { return '• (' + String(s.file || 'fájl') + ') „' + String(s.text).slice(0, 2000) + '"'; }).join('\n') + ']\n\n' + full;
    ...
    // 🕰 timeline cursor time (once per render) — drives the future-dim on nodes + the v2 edge-flow when a card crosses the playhead. Null in depth mode (x = generation, not time → no playhead → no time-keyed reveal/flow).
    var tlCurT = (tlOn && g.tl && tlX !== 'depth') ? (g.tl.tMin + tlCur * (g.tl.tMax - g.tl.tMin)) : null;
    var tlFlowWin = (tlCurT != null) ? (g.tl.tMax - g.tl.tMin) * 0.03 : 0;
    ...
    // P1.5 — preview which cards a hovered frame header would carry (center-inside), unless it's an in-flight drag
    var hovCarry = {};
    if (hovFrame && !(frdrag.current && frdrag.current.moved)) { var _hf = frames.filter(function (x) { return x.id === hovFrame; })[0]; if (_hf) g.N.forEach(function (n) { if (!n.mapHidden && inFrame(n, _hf)) hovCarry[n.id] = 1; }); }
    ...
    return h('div', { className: 'rmap-wrap' },
      ...
      h('div', { className: 'rmap-stage', ref: stageRef, onMouseDown: onDown, onWheel: onWheel, onMouseMove: broadcastCursor, onDoubleClick: onStageDbl },
        ...
        h('div', { className: 'rmap-world rmap-lod-' + lod + (smartAnim ? ' rmap-anim' : '') + (tlOn ? ' rmap-tl-on' : ''), style: { transform: 'translate(' + view.tx + 'px,' + view.ty + 'px) scale(' + view.k + ')' } },
        ...
        h('div', { className: 'rmap-presence', onMouseDown: function (e) { e.stopPropagation(); }, onWheel: function (e) { e.stopPropagation(); } },
          (online.length > 1) ? h('div', { className: 'rmap-avstack', title: 'Kattints egy kollégára a követéshez' },
            online.slice(0, 5).map(function (u, i) { var _a = avatarOf(u.id); return u.self ? h('div', { key: u.id, className: 'rmap-av me', style: { zIndex: 10 - i, background: _a ? undefined : colorOf(u.id) }, title: (u.name || 'Kolléga') + ' (te)' }, _a ? h('img', { src: _a, alt: '' }) : monogram(u.name)) : h('button', { key: u.id, className: 'rmap-av rmap-av-btn' + (following === u.id ? ' rmap-av-follow' : ''), style: { zIndex: 10 - i, background: _a ? undefined : colorOf(u.id), boxShadow: following === u.id ? '0 0 0 2px ' + colorOf(u.id) : null }, title: (following === u.id ? 'Követés leállítása: ' : 'Követés: ') + (u.name || 'Kolléga'), onClick: function () { toggleFollow(u.id); } }, _a ? h('img', { src: _a, alt: '' }) : monogram(u.name)); }),
        ...
        (function () {
          ...
          var nHidTypes = Object.keys(hiddenTypes).length, arrCaret = props.canEdit && !tlOn && !phaseArr;
          // g1 — VIEW menu + the arrange master-switch (with a ▾ caret opening the arrange-actions menu when in Szabad)
          var g1 = [
            h('button', { key: 'view', className: 'rmap-dbtn' + (viewMenu ? ' on' : ''), title: 'Nézet & panelek — szűrő, fájlok, export, frissítés, túrák, rejtett elemek', onClick: function (e) { openDMenu('view', e); } }, '👁' + (nHidTypes ? nHidTypes : ''), L('Nézet ▾')),
            (g.N.length > 1) ? h('div', { key: 'arr', className: 'rmap-arrpill' },
              h('div', { className: 'rmap-arrpill-seg' }, [['free', '⤫', 'Szabad'], ['phase', '⌗', 'Fázis'], ['time', '🕰', 'Idő']].map(function (o) {
                ...
                var tt = o[0] === 'free' ? 'Szabad — a kézi (mentett) elrendezés' : (o[0] === 'phase' ? 'Fázis — efemer fázis-oszlopok (nem írja felül a mentett elrendezést; ez NEM a menü ⌗ Fázisokba parancsa)' : 'Idő — idővonal: keletkezési idő szerint (efemer)');
                return h('button', { key: o[0], className: (on ? 'on' : ''), title: tt, onClick: function () { arrSet(o[0]); } }, o[1], ' ', o[2]);
              })),
              arrCaret ? h('button', { key: 'car', className: 'rmap-arrpill-car' + (arrMenu ? ' on' : ''), title: 'Elrendezés-akciók (Okos, Rendezd el, Fázisokba, keret…)', onClick: function (e) { openDMenu('arr', e); } }, '▾') : null) : null
          ].filter(Boolean);
          // g2 — EDIT: undo/redo (segmented) + history + comment — high-frequency, stay inline (never behind a menu)
          var g2 = [
            props.canEdit ? h('div', { key: 'ur', className: 'rmap-ur' },
              h('button', { className: 'rmap-ur-b', disabled: !histRef.current.undo.length, title: 'Visszavonás (⌘Z)' + (histRef.current.undo.length ? ' — ' + (histRef.current.undo[histRef.current.undo.length - 1].label || '') : ''), onClick: doUndo }, '↶'),
              h('button', { className: 'rmap-ur-b', disabled: !histRef.current.redo.length, title: 'Újra (⌘⇧Z)', onClick: doRedo }, '↷')) : null,
            ...
            commentsCap ? h('button', { key: 'cm', className: 'rmap-dbtn' + (commentMode ? ' on' : ''), title: commentMode ? 'Komment-mód kikapcsolása' : 'Komment-mód: kattints a vászonra vagy egy kártyára', onClick: function () { setCommentMode(function (v) { return !v; }); setComposer(null); } }, '💬' + (cmUnresolved || ''), L('Komment')) : null
          ].filter(Boolean);
          // g3 — ZOOM: a single compact widget  − [nn% ▾] +  (the ▾ opens Nézetbe + presets)
          var g3 = [
            h('div', { key: 'zoomw', className: 'rmap-zoomw' },
              h('button', { className: 'rmap-zoomw-b', title: 'Kicsinyítés', onClick: function () { zoom(0.85); } }, '−'),
              h('button', { className: 'rmap-zoomw-pct' + (zoomMenu ? ' on' : ''), title: 'Nagyítás: illesztés a nézetbe + előbeállítások', onClick: function (e) { openDMenu('zoom', e); } }, Math.round(view.k * 100) + '%', h('span', { className: 'rmap-zoomw-car' }, '▾')),
              h('button', { className: 'rmap-zoomw-b', title: 'Nagyítás', onClick: function () { zoom(1.18); } }, '+'))
          ];
          var kids = [];
        ...
        // decluttered-dock popover (Nézet / Elrendezés-akciók / Nagyítás) — one at a time, anchored under its trigger
        renderDMenu(),
          ...
          h('div', { className: 'modal', style: { width: 468, maxWidth: '94vw' }, onClick: function (e) { e.stopPropagation(); } },
            ...
            h('div', { style: { padding: 16, display: 'flex', flexDirection: 'column', gap: 14 } },
              h('div', { className: 'rmap-share-presence' },
                ...
                h('div', { className: 'rmap-avstack' }, online.slice(0, 8).map(function (u) { var _a = avatarOf(u.id); return h('div', { key: u.id, className: 'rmap-av' + (u.self ? ' me' : ''), style: { background: _a ? undefined : colorOf(u.id) }, title: (u.name || 'Kolléga') + (u.self ? ' (te)' : '') }, _a ? h('img', { src: _a, alt: '' }) : monogram(u.name)); }))),
                ...
                : h('div', { style: { display: 'flex', flexDirection: 'column', gap: 8 } },
                  ...
                  members.length ? members.map(function (m) {
                    return h('div', { key: m.user_id, className: 'rmap-mem-row' },
                      h('div', { className: 'rmap-av', style: { background: m.pavatar ? undefined : colorOf(m.user_id) } }, m.pavatar ? h('img', { src: m.pavatar, alt: '' }) : monogram(m.pname)),
                  ...
                  isOwner ? h('div', { className: 'rmap-invite' },
                    ...
                    invSel ? h('div', { className: 'rmap-invite-sel' },
                        h('div', { className: 'rmap-av', style: { width: 26, height: 26, fontSize: 12, background: colorOf(invSel.id) } }, monogram(invSel.name)),
