...
(function () {
  ...
  function css() {
    ...
    s.textContent =
      ...
      + '#pr-ob .card{width:480px;max-width:100%;background:var(--surface);border-radius:18px;box-shadow:0 24px 64px rgba(20,24,40,.2);padding:34px 34px 28px;margin:auto}'
      ...
      + '#pr-ob h1{font-size:22px;margin:0 0 6px;letter-spacing:-.3px;color:var(--ink)}'
      + '#pr-ob p.sub{font-size:14px;color:var(--muted);margin:0 0 22px;line-height:1.55}'
      + '#pr-ob .who{display:flex;align-items:center;gap:11px;background:var(--surface-2);border:1px solid #e6e8ee;border-radius:12px;padding:10px 13px;margin-bottom:20px}'
      ...
      + '#pr-ob .hint{font-size:11.5px;color:var(--muted);margin-top:5px}'
      + '#pr-ob .err{font-size:11.5px;color:var(--danger);margin-top:5px;display:none}'
      + '#pr-ob .err.on{display:block}'
      + '#pr-ob .menu{position:absolute;left:0;right:0;top:70px;z-index:5;background:var(--surface);border:1px solid #e6e8ee;border-radius:11px;box-shadow:0 12px 32px rgba(16,24,40,.14);max-height:230px;overflow:auto;display:none}'
      + '#pr-ob .menu.on{display:block}'
      + '#pr-ob .menu .it{padding:9px 13px;font-size:13.5px;color:var(--ink);cursor:pointer;border-bottom:1px solid var(--line,#f1f2f5)}'
      + '#pr-ob .menu .it:last-child{border-bottom:0}'
      + '#pr-ob .menu .it:hover,#pr-ob .menu .it.on{background:var(--accent-tint);color:var(--accent)}'
      + '#pr-ob .menu .add{color:var(--accent);font-weight:600}'
      ...
      + '#pr-ob .ghost{display:block;margin:14px auto 0;border:0;background:none;color:var(--muted);font-family:inherit;font-size:12.5px;font-weight:600;cursor:pointer}'
  ...
  function form() {
    var html = ''
      ...
      + '  <label for="ob-cc">Költséghely <span class="opt">— John von Neumann University</span></label>'
      + '  <input id="ob-cc" autocomplete="off" placeholder="Keresés név vagy kód szerint…" />'
      + '  <div class="menu" id="ob-cc-menu"></div>'
      + '  <div class="err" id="ob-cc-err">Válassz költséghelyet a listából.</div>'
      + '</div>'
      + '<div class="field">'
    ...
    aff.addEventListener('input', function () { aff.classList.remove('bad'); affErr.classList.remove('on'); render(aff.value); syncCC(); });
    menu.addEventListener('mousedown', function (e) {
      var it = e.target.closest('.it'); if (!it) return; e.preventDefault();
      aff.value = it.getAttribute('data-v'); menu.classList.remove('on'); syncCC();
    });
  ...
  function submit() {
    ...
    var ccIn = document.getElementById('ob-cc');
  ...
  function check() {
    sb.from('profiles').select('status,role,affiliation,mtmt_id,orcid,cost_center_code,cost_center').eq('id', me.id).maybeSingle().then(function (r) {
      var d = (r && r.data) || {};
      ...
      me.cost_center_code = d.cost_center_code || ''; me.cost_center = d.cost_center || '';
