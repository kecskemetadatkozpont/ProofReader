...
language sql stable security definer set search_path = public as $$
  select not coalesce((course_team_cfg(cid)->>'locked')::boolean, false)
     -- nullif: egy üres string dátummá alakítása hibát dobna
     and coalesce(nullif(course_team_cfg(cid)->>'deadline', '')::date >= current_date, true);
$$;
...
begin
  ...
  -- kurzusra szűrve: ugyanaz a hallgató több kurzuson is lehet csapattag
...
revoke all on function public.course_team_set_role(uuid, uuid, text)          from public, anon;
...
grant execute on function
  ...
  public.course_team_set_role(uuid, uuid, text), public.course_team_set_responsibility(uuid, text),
